...
import { useCostList, useUpdateCostList } from '@/api/costLists'
...
export default function CostListDetail() {
  const { id } = useParams<{ id: string }>()
  const navigate = useNavigate()
  const costListId = parseInt(id || '0', 10)

  const { data: costList, isLoading } = useCostList(costListId)
  ...
  usePageTitle(costList ? `Cost List - ${costList.vendor_name} / ${costList.item_sku}` : 'Cost List')

  useEffect(() => {
    if (isEditing && costList) {
      setFormData({
        vendor: String(costList.vendor),
        item: String(costList.item),
        begin_date: costList.begin_date,
        end_date: costList.end_date ?? '',
        is_active: costList.is_active,
        notes: costList.notes || '',
      })
      setLines(
        (costList.lines ?? []).map((line) => ({
          id: line.id,
  ...
  const handleAddLine = () => {
    setLines([...lines, { min_quantity: '1', unit_cost: '0.00' }])
  }
  ...
  if (!costList) {
    return (
      <div className="raven-page" style={{ minHeight: '100vh' }}>
        <div className="max-w-[1080px] mx-auto px-4 md:px-8 py-7 pb-16">
          <div className="text-center py-8" style={{ color: 'var(--so-text-tertiary)' }}>Cost list not found</div>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-[1080px] mx-auto px-4 md:px-8 py-7 pb-16">
        ...
        <div className="flex items-center gap-2 mb-5 animate-in">
          <button
            ...
            <ArrowLeft className="h-3.5 w-3.5" />Cost Lists
          </button>
          <span style={{ color: 'var(--so-border)' }} className="text-[13px]">/</span>
          <span className="text-[13px] font-medium" style={{ color: 'var(--so-text-secondary)' }}>{costList.vendor_name} / {costList.item_sku}</span>
        </div>
        ...
        <div className="flex flex-wrap items-start justify-between gap-3 mb-7 animate-in delay-1">
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-bold" style={{ letterSpacing: '-0.03em' }}>Cost List</h1>
              {getStatusBadge(costList.is_active ? 'active' : 'inactive')}
            </div>
            <div className="text-sm" style={{ color: 'var(--so-text-secondary)' }}>
              <button
                onClick={() => navigate(`/vendors/${costList.vendor}`)}
                ...
                {costList.vendor_name}
              </button>
              {' · '}
              <button
                onClick={() => navigate(`/items/${costList.item}`)}
          ...
          <div className="flex flex-wrap items-center gap-2" data-print-hide>
            {isEditing ? (
              <>
                ...
                <button className={primaryBtnClass} style={primaryBtnStyle} onClick={handleSave} disabled={updateCostList.isPending}>
                  <Save className="h-3.5 w-3.5" />{updateCostList.isPending ? 'Saving...' : 'Save Changes'}
                </button>
              </>
            ) : (
              <>
        ...
        <div className="rounded-[14px] border overflow-hidden mb-4 animate-in delay-2" style={{ background: 'var(--so-surface)', borderColor: 'var(--so-border)' }}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4">
            {[
              { label: 'Vendor', value: costList.vendor_name, sub: costList.vendor_code, mono: false },
              { label: 'Item', value: costList.item_sku, sub: costList.item_name, mono: true },
              { label: 'Valid From', value: formatDate(costList.begin_date), mono: false },
              { label: 'Tiers', value: String(costList.lines?.length ?? 0), mono: true },
            ].map((kpi, idx) => (
        ...
        {/* Details Card */}
        <div className="rounded-[14px] border mb-4 animate-in delay-2" style={{ background: 'var(--so-surface)', borderColor: 'var(--so-border)', position: isEditing ? 'relative' : undefined, zIndex: isEditing ? 20 : undefined }}>
          <div className="px-6 py-4" style={{ borderBottom: '1px solid var(--so-border-light)' }}>
              ...
              <div className="space-y-4">
                <div className="grid gap-4 md:grid-cols-2">
                  <div className="space-y-2">
                    <Label className="text-[12px] font-medium uppercase tracking-wider" style={{ color: 'var(--so-text-tertiary)' }}>Vendor</Label>
                    <SearchableCombobox
                      entityType="vendor"
              ...
              <div className="space-y-5">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                  <div>
                    ...
                    <div className="text-sm font-medium" style={{ color: 'var(--so-text-primary)' }}>{formatDate(costList.begin_date)}</div>
                  </div>
                  <div>
                    <div className="text-[11.5px] font-medium uppercase tracking-widest mb-1.5" style={{ color: 'var(--so-text-tertiary)' }}>End Date</div>
                    <div className="text-sm font-medium" style={{ color: 'var(--so-text-primary)' }}>{formatDate(costList.end_date)}</div>
                  </div>
                  <div>
                    <div className="text-[11.5px] font-medium uppercase tracking-widest mb-1.5" style={{ color: 'var(--so-text-tertiary)' }}>Created</div>
                    <div className="text-sm" style={{ color: 'var(--so-text-primary)' }}>{format(new Date(costList.created_at), 'MMM d, yyyy')}</div>
                  </div>
                  <div>
                    <div className="text-[11.5px] font-medium uppercase tracking-widest mb-1.5" style={{ color: 'var(--so-text-tertiary)' }}>Last Updated</div>
                    <div className="text-sm" style={{ color: 'var(--so-text-primary)' }}>{format(new Date(costList.updated_at), 'MMM d, yyyy')}</div>
                  </div>
                </div>
                {costList.notes && (
                  <div>
                    <div className="text-[11.5px] font-medium uppercase tracking-widest mb-1.5" style={{ color: 'var(--so-text-tertiary)' }}>Notes</div>
                    <div className="text-sm whitespace-pre-wrap rounded-lg px-4 py-3" style={{ background: 'var(--so-bg)', color: 'var(--so-text-secondary)' }}>{costList.notes}</div>
                  </div>
                )}
              </div>
            )}
            ...
            {isEditing ? (
                ...
                <div className="space-y-3">
                  {lines.map((line, index) => (
                    <div key={index} className="grid grid-cols-12 gap-2 items-end p-3 rounded-[10px]" style={{ background: 'var(--so-bg)' }}>
                      ...
                      <div className="col-span-5 space-y-1">
                        <Label className="text-xs" style={{ color: 'var(--so-text-tertiary)' }}>Unit Cost</Label>
                        <Input
                          type="number"
                          step="0.0001"
                          min="0"
              ...
              <div className="rounded-[10px] overflow-hidden" style={{ border: '1px solid var(--so-border)' }}>
                <table className="w-full">
                  ...
                  <tbody>
                    {costList.lines && costList.lines.length > 0 ? (
                      costList.lines.map((line, idx) => (
                        <tr key={line.id} style={{ borderBottom: idx < costList.lines!.length - 1 ? '1px solid var(--so-border-light)' : 'none' }}>
