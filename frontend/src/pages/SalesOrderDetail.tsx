...
export default function SalesOrderDetail() {
  ...
  activeContracts.forEach(contract => {
    contract.lines?.forEach(cl => {
      ...
      options.push({
        ...
        unitPrice: cl.unit_price as string,
  ...
  const { data: priceData, isFetching: isPriceFetching } = usePriceLookup(
    order?.customer ? Number(order.customer) : undefined,
  ...
  const handleLineItemChange = (index: number, value: string) => {
    ...
    if (value && order?.customer && !bestContract) {
  ...
  const handleLineQtyChange = (index: number, value: string) => {
    ...
    if (value && order?.customer && linesFormData[index]?.item && !hasContract) {
      setPriceLookupLine(index)
    }
  }
  ...
  if (!order) {
    return (
      <div className="so-detail-page" style={{ minHeight: '100vh' }}>
        <div className="max-w-[1080px] mx-auto px-4 md:px-8 py-7">
          <div className="text-center py-16 text-sm" style={{ color: 'var(--so-text-tertiary)' }}>Sales order not found</div>
        </div>
      </div>
    )
  }
  ...
  const detailItems = isEditing
    ? [
        ...
        { label: 'Bill To', value: order.bill_to_name || 'Not set', empty: !order.bill_to_name, mono: false, editable: false },
      ]
    : [
        ...
        { label: 'Customer PO', value: order.customer_po || 'Not set', empty: !order.customer_po, mono: true },
        ...
        { label: 'Bill To', value: order.bill_to_name || 'Not set', empty: !order.bill_to_name, mono: false },
      ]
    ...
    <div className="so-detail-page" style={{ minHeight: '100vh' }}>
      ...
      <div className="max-w-[1080px] mx-auto px-4 md:px-8 py-7 pb-16" data-print-hide>
        ...
        <div className="flex items-center gap-2 mb-5 animate-in">
          <button
            onClick={() => navigate('/customers/open-orders')}
        ...
        {/* ── Title row ──────────────────────────── */}
        <div className="flex flex-wrap items-start justify-between gap-3 mb-7 animate-in delay-2">
        ...
        <div className="rounded-[14px] border overflow-hidden mb-4 animate-in delay-2" style={{ background: 'var(--so-surface)', borderColor: 'var(--so-border)' }}>
          ...
          {/* Contract reference */}
          {!isEditing && order.contract_reference && (
            <div
            ...
            >
              ...
              >
                Contract: {order.contract_reference.contract_number}
              </span>
                    ...
                    {order.lines.map((line) => (
                      <tr key={line.id} style={{ borderBottom: '1px solid var(--so-border-light)' }}>
                        ...
                        <td className="py-3.5 px-4">
                          {line.contract_number ? (
                            <span className="font-mono text-[12.5px]" style={{ color: 'var(--so-accent)' }}>{line.contract_number}</span>
                          ) : (
